...
public class SQLiteDatabase extends SQLiteOpenHelper {
    ...
    public boolean UpdateRecords(String fname, String mname, String lname, Integer id){
        android.database.sqlite.SQLiteDatabase conn= this.getWritableDatabase();
        sql="UPDATE " + PROFILE +" SET "+ PROFILE_FNAME + " = '"+fname+"', "+ PROFILE_MNAME + " =
        '"+mname+"',"+ PROFILE_LNAME + " = '"+lname+"' WHERE " + PROFILE_ID + "="+id ;
        conn.execSQL(sql);
        return true;
    }
    ...
    public ArrayList<String>GetAllData(){
        android.database.sqlite.SQLiteDatabase conn= this.getReadableDatabase();
        Records=new ArrayList<String>(); RecordsId=new ArrayList<Integer>();
        sql="SELECT * FROM " + PROFILE;
        rs=conn.rawQuery(sql, null); rs.moveToFirst();
        while(rs.isAfterLast()==false){
            RecordsId.add(rs.getInt(rs.getColumnIndex(PROFILE_ID)));
            Records.add(rs.getString(rs.getColumnIndex(PROFILE_LNAME)) + ",
                    "+rs.getString(rs.getColumnIndex(PROFILE_FNAME))+ " "+rs.getString(rs.getColumnIndex(PROFILE_MNAME)));
                    rs.moveToNext()
        }return Records;
    }
}
